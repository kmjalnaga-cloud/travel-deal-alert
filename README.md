...
숙소에 `dates`를 추가하면 오늘 1박 점검과 별개로 해당 날짜도 함께 확인한다.

```json
"dates": [
  { "label": "설연휴", "checkIn": "2027-02-06", "checkOut": "2027-02-07" }
]
```

- 아직 예약이 열리지 않았거나 매진이면 가격이 조회되지 않으므로 조용히 대기한다.
- 가격이 조회되기 시작하면(예약 오픈, 취소분 재오픈 포함) "예약 가능해졌어요"로 알린다. 매진 후 다시 열려도 새로 알린다.
- 날짜가 예약 불가일 때 붙어 있는 배지는 무시하고, 예약 가능한 동안 새로 뜨는 배지만 알린다.
- `label`은 숙소 안에서 겹치지 않게 짓는다(상태 저장 키로 쓰인다).
